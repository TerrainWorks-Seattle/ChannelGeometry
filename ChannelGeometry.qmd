...
```{r}
hab_width <- data[AC_WIDTH>0, .(AREA_SQKM, SPRAG14RCH, AC_WIDTH)]
hab_width <- hab_width[, .(width = mean(AC_WIDTH, na.rm = TRUE)), by = SPRAG14RCH]
width_habitat <- merge(widths, areas, by = "SPRAG14RCH")
# (Optionally) remove outliers
...
```{r}
mask_width <- data[MaskWidth>0, .(AREA_SQKM, SPRAG14HAB, MaskWidth)]
mask_width <- mask_width[, .(width = mean(MaskWidth, na.rm = TRUE)), by = SPRAG14HAB]
width_mask <- merge(widths, areas, by = "SPRAG14RCH")
